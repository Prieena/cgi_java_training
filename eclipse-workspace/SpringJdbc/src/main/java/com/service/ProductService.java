...
//		}
		List<Product> updatedProduct = listOfProduct.stream().map(p -> {float discount = p.getPrice()*0.10f; p.setPrice(p.getPrice()-discount); return p;}).collect(Collectors.toList()); 
